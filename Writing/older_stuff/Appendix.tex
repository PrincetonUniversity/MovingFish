...
We parameterize our model for black rockfish in the California Current, with MPAs of spacing and width that is qualitatively similar to the MLP Act reserves, and with a maximum climate velocity equal to that observed empirically. The parameters and references are as follows

\begin{table}[htdp]
\caption{Rockfish simulation parameters}
\begin{center}
\begin{tabular}{c l l}
\hline
...
We find qualitatively similar results with this parameterization to what is presented in the main text. In particular, our black rockfish parameterization has the same counterintuitive result that MPAs from which effort is displaced (rather than eliminated) can be worse under some rates of range shift and harvesting rates, than no MPA at all (i.e. comparing Fig. 2a and 2e). 
