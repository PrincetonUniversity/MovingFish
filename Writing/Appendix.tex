...
\subsection{Re-allocation of Effort \label{effort}}
~\\Spatial shifts of harvesters in response to MPA creation can affect sustainability of the system \citep{Botsfordetal2009}. We model the two extremes of response: either that harvesting pressure that occurred in now-MPAs is reallocated outside the reservers, or harvesting inside MPAs disappears from the system with the creation of MPAs and MPAs are effectively an effort control management technique. To reallocate effort we increased quota by 50\% in outside of MPAs (see \cite{Kaplanetal2005} for similar approach). 

We found no qualitative change in our results. Reallocation of effort was effectively like increasing the harvesting rate, and thus the critical harvest rate was lower than when effort was removed from the system. 

\begin{figure}[h]
\centering
\includegraphics[width=.75\textwidth]{plots/fig3a.pdf}
\caption{Climate velocity is on the x axis, harvest rate is on the y axis. These simulations were run with effort reallocated to areas outside of MPAs. }
% see bounded\_flux.R for code that made this plot
\end{figure}
