...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=.5\textwidth]{plots/critical_rates.pdf}
\caption{As the rate of environmental shift $c$ increases, the critical harvesting rate $h^*$ decreases.  In other words, less harvesting pressure is required to drive the population extinct.  Increasing the growth rate always increases the critical harvesting rate.  On the other hand, increasing average dispersal distance increases the critical harvesting rate depends on how quickly the environment is shifting: it is more advantageous to disperse farther when the environment is shifting more quickly.  The rate of environmental shift is plotted on the x-axis and the critical harvesting rate is plotted on the y-axis.  Black lines correspond to a growth rate of $R_0=3$, red to $R_0=7$, and blue to $R_0=10$.  Solid lines correspond to an average dispersal distance $\langle d \rangle =0.1$ and dashed lines correspond to an average dispersal distance $\langle d \rangle =0.25$.  These results are from a Gaussian dispersal kernel with $L=1$.}
\label{critical}
\end{center}
\end{figure}
...
%\includegraphics[width=.45\textwidth]{plots/eqbiomass_mpa_v2.pdf}
