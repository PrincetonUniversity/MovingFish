...
sims <- data.frame(model = c("noThresh","noThresh","noThresh","Thresh"),
					MPA = c("rock","rock","null","null"),
					effort_allocate = c(NA, "yes", NA, NA), stringsAsFactors=FALSE)
...
# run analysis
	...
	# if no threshold, choose MPA: "null", "rock"
	MPA = sims$MPA[run]
