...
sims <- data.frame(model = c("noThresh","noThresh","noThresh","noThresh","noThresh","noThresh"),
					MPA = c("cons","fish","null","cons","fish","null"),
					effort_allocate = c(NA, NA, NA, "yes", "yes", NA), stringsAsFactors=FALSE)
...
# analysis
timed <- system.time(
	if(model=="noThresh") {sapply(c("Parameters_nothresh.R","fluctuations_sim.R"),source,.GlobalEnv)} else {
