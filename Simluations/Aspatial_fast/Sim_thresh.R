...
for(q in 1:length(speeds)){
  ...
  for(j in 1:length(thresholds)){
    ...
    while(h.diff > init.diff | roll.mean > init.diff){
      T = T + 1
      next.pop <- m(n=init.h[,T-1], s = 0, Fthresh=thresholds[j], Fharv = 1, mpa.yes = mpa.yes, mpa.no = mpa.no, MPA.current = MPA.current)
    ...
    move <- array(0,c(w,1))
    next.pop <- m(n=init.h[,T], s = speeds[q], Fthresh=thresholds[j], Fharv = 1, mpa.yes = mpa.yes, mpa.no = mpa.no, MPA.current = MPA.current)
    move[,1] <- next.pop[[1]]
    MPA.current <- next.pop[[3]]
    ...
    while(h.diff > init.diff & roll.mean > init.diff){
      T = T + 1
      next.pop <- m(n=move[,T-1], s = speeds[q], Fthresh=thresholds[j], Fharv = 1, mpa.yes = mpa.yes, mpa.no = mpa.no, MPA.current = MPA.current)
    ...
    for (i in (T+1):(T+300)){
      next.pop <- m(n=move[,i-1], s = speeds[q], Fthresh=thresholds[j], Fharv = 1, mpa.yes = mpa.yes, mpa.no = mpa.no, MPA.current = MPA.current)
      move <- cbind(move, next.pop[[1]])
      MPA.current = next.pop[[3]]
